      ...
      <div>
        ...
        <div className="relative aspect-video overflow-hidden bg-gradient-to-br from-indigo-900 to-purple-900">
          ...
          <div className="absolute top-3 left-3 flex gap-2">
            ...
            {showActions && (
              <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${statusColors[course.status] || 'bg-gray-100'}`}>
                {course.status}
              </span>
            )}
          </div>
        ...
        <div className="p-5">
          ...
          {course.instructor && (
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              by {typeof course.instructor === 'object' ? `${course.instructor.firstName || ''} ${course.instructor.lastName || ''}`.trim() : 'Instructor'}
            </p>
          )}
